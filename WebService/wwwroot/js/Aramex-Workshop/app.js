import { $, $$, to24HourFormat, formatRangeLabel, toDateInputFormat, loadingFadeIn, loadingFadeOut } from './helpers.js';
...
async function calculateStemInOutBounds() {
    try {
        writeLog(logLevels.info, 'calculateStemInOutBounds has started.', '');
        loadingFadeIn();
        markersGroup.removeAll();

        AramexInitials.StemTrip.Oubound.destination.forEach(function (currentValue, index, arr) {
            var marker = new H.map.Marker(currentValue.displayLocation, {
...
export { router, geocoder, calculateStemInOutBounds, calculateIsoline, markersGroup }
