import { $ } from './helpers.js';
import { autocompleteGeocodeUrl, requestGeocode } from './here.js';
import { HEREInitials } from './config.js';
import { calculateIsoline, calculateRoute, markersGroup } from './app.js';

class Search {

    constructor(startLocation) {
        this.active = 0;
        this.matches = [];
        this.container = $('.search-container');
        this.input = $('.city-field');
        this.input.innerText = startLocation;
        this.input.oninput = (evt) => this.updateField(evt);
        this.input.onkeydown = (evt) => this.onkeyinput(evt);
        this.label = '';
    }

    async updateField(evt) {
        const value = evt.target.innerText;
        if (value.length === 0) {
            return;
        }

        this.matches = await fetch(autocompleteGeocodeUrl(value)).then(res => res.json());
        //const match = this.matches.suggestions.filter(x => x.matchLevel === 'city')[0];
        const match = this.matches.results.filter(x => x.resultType === 'place')[0];

        if (match === undefined) {
            $('.city-field-suggestion').innerText = '';
        } else {
            //this.label = match.address.city + ', ' + match.countryCode;
            this.label = match.title + ', ' + match.vicinity;
            //this.active = match.locationId;
            this.active = match.position;
            $('.city-field-suggestion').innerText = value + this.label.substring(value.length, this.label.length);
        }
    }

    onkeyinput(evt) {
        const code = evt.keyCode;
        if (code === 13 || code === 9) {
            $('.city-field').innerText = this.label;
            $('.city-field-suggestion').innerText = '';
            evt.preventDefault();
            this.selectMatch();
        }
    }

    async selectMatch() {
        //const { Latitude: lat, Longitude: lng } = await requestGeocode(this.active);
        //const { Latitude: lat, Longitude: lng } = this.active //await requestGeocode(this.active);
        //HEREInitials.Center.lat = lat;
        //HEREInitials.Center.lng = lng;

        HEREInitials.Center.lat = this.active[0];
        HEREInitials.Center.lng = this.active[1];
        
        markersGroup.removeAll();

        var selectMatchMarker = new H.map.Marker(HEREInitials.Center, {
            icon: HEREInitials.Markers.Icons.LocationIcon
        });

        markersGroup.addObject(selectMatchMarker);
        map.addObject(markersGroup);

        // get geo bounding box for the group and set it to the map
        map.getViewModel().setLookAtData({
            bounds: markersGroup.getBoundingBox()
        });

        //calculateIsoline();
        //calculateRoute();
        //make_QRCode();
    }

}

export default Search;